...
DT::datatable(notas, class = 'cell-border stripe',
              rownames = F,
              #filter = 'top',
              editable = T
              )
...
<center>
```{r, echo=FALSE}
pander::pander(posicao(notas$S1))
pander::pander(posicao(notas$S2))
```
</center>
...
p1 = ggplot(notas) + 
  geom_histogram(aes(x=S1,
                     y=..density..), 
                 position="identity",
                 col = 1,
                 lwd = 1,
                 breaks = hist(notas$S1, plot = FALSE)$breaks) + 
  geom_density(aes(x=S1,
                   y=..density..),
               col = 4,
               lwd = 1) + 
  ylab("Densidade") +
